...
def main(opts):
    ...
    while cur_epoch < opts.epochs and not opts.test:
        ...
        if (cur_epoch + 1) % opts.val_interval == 0:
            ...
            for k, (img, target, reconst) in enumerate(ret_samples):
    ...
    if rank == 0:  # save best model at the last iteration
        # best model to build incremental steps
        if not opts.debug:
            save_ckpt(checkpoint_path, model, cur_epoch, best_score)
            logger.info("[!] Checkpoint saved.")
    ...
    val_loss, val_score, _ = model.validate(loader=test_loader, metrics=val_metrics, logger=logger)
